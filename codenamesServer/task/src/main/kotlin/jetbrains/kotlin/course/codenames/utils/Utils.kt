...
object Utils {
    ...
    val uniqueKeyCardGenerator: KeyCardGenerator = KeyCardGenerator {
        var keyCardCells: List<KeyCardCell> = listOf()

        do {
            keyCardCells = KeyCardType.values().map { key ->
                List(key.number) {
                    KeyCardCell(key)
                }
            }.flatten().shuffled()
        } while (keyCardCells in previousAttempts)

        previousAttempts.add(keyCardCells)
        keyCardCells
    }
